...
class ilStudyProgrammeUserProgress
{
	 ...
	 */
	public function markFailed(int $user_id) : ilStudyProgrammeUserProgress
	{
		$status = array(
	 ...
	 */
	public function updateFromProgramNode() : bool
	{
	 ...
	 * progress.
	 */
	public function setLPCompleted(int $obj_id, int $usr_id) : void 
	{
	...
	{
		if(
			$this->getStudyProgramme()->getId() == $this->assignment_repository->read(
				$this->getAssignmentId())->getRootId()
		) {
			return [];
		}
	...
	static public function getPossibleActions(
		...
		} else if ($status == ilStudyProgrammeProgress::STATUS_IN_PROGRESS) {
     ...
     * @throws ilException
     */
	public function sendRiskyToFailMail(int $progress_id, int $usr_id) : void
    {
        global $DIC;
        $lng = $DIC['lng'];
